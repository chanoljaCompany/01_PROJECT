...
<form name = "nboard" method="post" enctype="multipart/form-data">
 ...
 <input type="hidden" name="left_menu_id" id="left_menu_id" value="<?=$left_menu_id?>">
	<div class="box_title first">
		<h2 class="title">여행후기 글작성</h2>
	</div>
	<table class="tbl_row multi_shop">
		<caption class="hidden">여행후기 글작성</caption>
		<colgroup>
			<col style="width:15%">
			<col style="width:85%">
		</colgroup>
        ...
        ?>
               ...
               <ul>
                    <li>
                        ...
                        <a href="<?=$file_url?>/community/travel_reviews/nboard_file_download.php?original_table=<?=$value['original_table']?>&seq=<?=$value['seq']?>&no=0&page=<?=$page?>&top_menu_id=<?=$top_menu_id?>&left_menu_id=<?=$left_menu_id?>" class="view_file_download">
