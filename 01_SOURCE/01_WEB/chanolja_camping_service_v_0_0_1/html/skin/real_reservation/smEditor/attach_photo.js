	...
	var nTotalSize = 0;
	var nMaxImageSize = 1*1024*1024;  //김유혁수정
	var nMaxTotalImageSize = 50*1024*1024;  //김유혁수정
    ...
    function html5Upload() {
    	...
    	// sUploadURL= 'file_uploader_html5.php'; 	//upload URL
			// sUploadURL= sUploadURLConf; 	//upload URL
			//sUploadURL= '../../../../guestroom/file_uploader_html5.php'; 	//upload URL
			sUploadURL = SmEditor_url;  //김유혁수정
    	//파일을 하나씩 보내고, 결과를 받음.
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
    	var oAjax = jindo.$Ajax(sUploadURL, {
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				// alert('sResString ' + sResString);  //김유혁수정
				if (res.readyState() == 4) {
		...
		oAjax.header("file-pageId",pageId);
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			  sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader_html5.php',	//샘플 URL입니다.
