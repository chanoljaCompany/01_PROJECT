...
cars['홀덴->Tigra'] = '컨버터블';
    ...
    function makechange() 
    {
        ...
        modelview(f.make.value, f.brand.value);
    }
	...
	var model = document.fsearch_cars.model.value;
	...
	if (make == "" && brand == ""&& model == "") {
		...
		if (brand == "전체")  {
			document.fsearch_cars.stx.value = make;
		} else {
		   if (brand == "전체"&& model == "전체")  {
			document.fsearch_cars.stx.value = make + " " + brand;
			}else {
			document.fsearch_cars.stx.value = make + " " + brand + " " + model;
			}
		}
