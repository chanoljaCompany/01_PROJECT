...
if ($w == 'u' || $w == 'r') { //글수정,답변글일경우 해당글을 가져옴...
...
if ($w == '' || $w == 'r') {	//새글,답변글일 경우
		if ($session_userid) { // 회원..관리자...
			 ...
			 $wr_homepage = clean_xss_tags($wr_homepage);
				 ...
				 // 새글 INSERT
					sql_query(" insert into {$g5['board_new_table']} ( bo_table, wr_id, wr_parent, bn_datetime, mb_id ) VALUES ( '{$NBOARD_TB}', '{$wr_id}', '{$wr_id}', '".G5_TIME_YMDHIS."', '{$pension_user_id}' ) ");
...
// 											";
									        ...
									        //데이터베이스 입력
													$sql = "INSERT INTO $NBOARD_FILE_TB
																	(original_table , file_code , original_seq , v_fileName , o_fileName , cnt , file_size , mod_ip , mod_date , pension_user_id)
																	VALUES
																	('$NBOARD_TB','$file_code','$wr_id','$fileName','$saveFileName','$cnt','$fileSize','$mod_ip','$mod_date','$session_userid')";
					...
					echo "<script>alert('공지사항을 등록하였습니다.');</script>";
