<script>
    ...
    var optbasicData;
    var form_userid;
    var areaArray = <?php echo json_encode($area)?>; 
    var itemlist_ca_id ;
    $(function(){
   ...
   onClose: function(selectedDates, dateStr, instance) {
    ...
    <?}else{?>
        ...
        if(pageDiv == 'reserve_first'){
            // alert('pageDiv ' + pageDiv);
            $("#subpagebg").hide();  
            findSearchData('sub')
        }
        else if(pageDiv == 'reserve_second'){
            var guestroom_code = $("#guestroom_code").val();
            $("#subpagebg").hide();  
            // alert('pageDiv ' + pageDiv + 'guestroom_code ' + guestroom_code);
            room_detail_view(guestroom_code);
        }
        else if(pageDiv == 'reserve_third'){
            // var guestroom_code = $("#guestroom_code").val();
            $("#subpagebg").hide();  
            // alert('pageDiv ' + pageDiv + 'guestroom_code ' + guestroom_code);
            // room_detail_view(guestroom_code);
        }
        else if(pageDiv == 'reserve_fourth'){
            // var guestroom_code = $("#guestroom_code").val();
            $("#subpagebg").hide();  
            $("#login_form").hide();  
            $("#register_form").hide();  
      ...
      console.log('get_area========');
       ...
       var html = "";
        for(var i=0 ; i < areaData.length ;i++ ){
            var chkStr = "";
            if(areaArray[i] == areaData[i]['area_code']){
                chkStr = "checked";
            }
            // alert(areaArray[i]);
            html += "<span>";
            html += "<input type='checkbox' name='area[]' value='"+areaData[i]['area_code']+"' class='' "+chkStr+">";
...
if(divisionType == '1') {
  ...
  if(datalen < '11'){
    ...
    $("#division").val("getData");
    $("#locate").val(locate);
    $("#area").val(areaArray);
    $("#basicoption").val(optionArray);
    if(locate == 'main'){
    $("#pageDiv").val("reserve_first");
    $("#reserveForm").attr("action","/bbs/board.php").submit();
    }
    else{
  ...
  $.ajax({
    ...
    //   else{
      ...
      console.log('reserve_first_area');
    //   }
    //   alert(dateStr);
      $("#dateStr").val(dateStr);
     }
...
//   alert('room_image ' + guestroom_code);
...
function room_detail(guestroom_code){
    // var dateStr = $("#dateStr").val();
    // location.href="/bbs/board.php?bo_table=73&pageDiv=reserve_second&guestroom_code="+guestroom_code+"&dateStr="+dateStr;
    $("#pageDiv").val("reserve_second");
    $("#guestroom_code").val(guestroom_code);
    $("#reserveForm").attr("action","/bbs/board.php").submit();  
}
function room_detail_view(guestroom_code){
    move_top();  
  var dateStr = $("#dateStr").val();
...
function rightData(){
    ...
    for(var i=0 ; i < room_info_data.length ;i++ ){
        ...
        $("#guestroom_code").val(room_info_data[i]['guestroom_code']);
    ...
    for(var i=0 ; i < contact.length ;i++ ){
        ...
        html_2 += "<td><span class=''>"+parseInt(contact[i]['option_fee']).toLocaleString()+"</span><span class=''>원</span></td>";
        html_2 += "</tr>";
...
$("#reserve_btn").click(function(){
    ...
    //    var url = '/bbs/board.php?bo_table=70'; 
    //    history.pushState(state, title, url);
    // $("#pageDiv").val("reserve_third");
    // $("#reserveForm").attr("action","/bbs/board.php").submit();
    // $("#subpagebg").hide();  
    <? if($_SESSION['session_id']){?>
        $("#pageDiv").val("reserve_fourth");
        $("#reserveForm").attr("action","/bbs/board.php").submit();  
    <?}?>
  $("#sub_page_detail").hide(); 
//   $("#customer_info").hide(); 
...
// $("#reserve_btn_2").click(function(){
function reserve_btn_2(){    
    //  alert('reserve_btn_2');
  move_top();
...
//   alert('discount_fee ' + discount_fee);
...
//       );
    ...
    //     });
    //     $("#select_option").val(seloptionArray);
      var seloptionArray =  $("#select_option").val().split(",");
      var option_info_data_size = seloptionArray.length; //옵션갯수..
      //  alert('seloptionArray ' + seloptionArray);
      var t=1;
      var optStr = "";
...
function reserve_fifth(reserve_code){
    $("#pageDiv").val("reserve_fifth");
    $("reserve_code").val(reserve_code);
    $("#reserveForm").attr("action","/bbs/board.php").submit(); 
}
function reserve_fifth_view(){
  var reserve_code = $("#reserve_code").val();
  $("#reservation_completion").show();
  $.ajax({
      type: 'GET',
...
function move_page(pmove){
  if(pmove == 'main'){
    location.href='/bbs/board.php?bo_table=73';
  }
  else if(pmove == 'first'){
    // $("#sec2").hide();
    // $("#sec1").show();
...
<? if($pageDiv == 'reserve_first' || $pageDiv == 'reserve_second'){?>
<script>
        function mapstart(){
