...
function get_option($userid,$option_type){
...
function get_paymethod_type($paymethod_type) { //결제방식
      ...
      else if($paymethod_type == '6'){
        $paymethod_type_val = "현장결제";
      }
      else{
...
function get_date_intval_admin($dateStr){ //날짜차이 계산...
  $dateStrExp =explode("~",$dateStr);
  $startDate = trim($dateStrExp['0']);
  $endDate = trim($dateStrExp['1']);
  $dateintval = (strtotime($endDate) - strtotime($startDate))/86400;
  return $dateintval;
}
...
function peak_season_update($PEAK_DATE_TB){ //성수기..준성수기 날짜 업데이트 // 관리자 로그인시 처리함.user_login.php
  $toYear = date("Y");
  $sql = "SELECT * FROM $PEAK_DATE_TB ORDER BY seq DESC LIMIT 0,1";
  $prows = sql_fetch($sql);
  if($PEAK_DATE_TB == 'management_reserve_peak_date'){ //피크시즌
    $dataStr1 = "peak_season_start_date";
    $dataStr2 = "peak_season_end_date";
  }
  else{
    $dataStr1 = "semi_peak_season_start_date";
    $dataStr2 = "semi_peak_season_end_date";
  }
  $season_start_date = $prows[$dataStr1];
  $season_end_date = $prows[$dataStr2];
  $season_start_date_ex = explode("-",$season_start_date);
  $season_end_date_ex = explode("-",$season_end_date);

  $start_date_year = $season_start_date_ex['0'];
  $start_date_month = $season_start_date_ex['1'];
  $start_date_day = $season_start_date_ex['2'];
  $end_date_year = $season_end_date_ex['0'];
  $end_date_month = $season_end_date_ex['1'];
  $end_date_day = $season_end_date_ex['2'];

  if($start_date_year != $toYear) {
    $updateStart = $toYear."-".$start_date_month."-".$start_date_day;
    $updateEnd = $toYear."-".$end_date_month."-".$end_date_day;
    $sql = "UPDATE $PEAK_DATE_TB
            SET
            $dataStr1 = '".$updateStart."'
           ,$dataStr2 = '".$updateEnd."'
           WHERE 1=1
           AND seq = '".$prows['seq']."'
    ";
...
function get_member_type($member_type) { //회원구분
    if($member_type == 'M'){
      $paymethod_type_val = "호스트";
    }
    else if($paymethod_type == 'C'){
      $paymethod_type_val = "일반회원";
    }
    else{
      $paymethod_type_val = "회원구분오류";
    }
    return $paymethod_type_val;
}

function get_goption_use($option_code,$guestroom_code,$userid){
    global $GUESTROOM_OPTION_TB;
    $sql = "SELECT COUNT(seq) AS Num FROM $GUESTROOM_OPTION_TB 
            WHERE 1=1
            AND user_id='".$userid."' AND option_code = '$option_code' AND guestroom_code='$guestroom_code' AND option_del_whether = 'Y'";
    $rows = sql_fetch($sql);
    $num = $rows['Num'];
    return  $num;
}
function get_garea_use($area_code,$guestroom_code,$userid){
    global $GUESTROOM_AREA_TB;
    $sql = "SELECT COUNT(seq) AS Num FROM $GUESTROOM_AREA_TB 
            WHERE 1=1
            AND user_id='".$userid."' 
...
function get_image_info($guestroom_code){
    global $GUESTROOM_IMAGE_INFO_TB;

$sql = "SELECT * FROM $GUESTROOM_IMAGE_INFO_TB
          WHERE 1=1
          AND guestroom_code = '$guestroom_code'
          ";
  $result = sql_query($sql);
      ...
      while ($rows = sql_fetch_array($result)) {
        ...
        array_push($image_data_array, $arrData);
    }
      return $image_data_array;
}

function get_reserve_peak_data($userid,$userlevel){ //호스트성수기날짜.
  global $MANAGEMENT_RESERVE_PEAK_DATE_TB;
...
function get_user_all($manager){ //회원종류에 따른 회원리스트
    global $SALES_MEMBERS_TB;
    $sql = "SELECT *
            FROM $SALES_MEMBERS_TB
            WHERE 1=1
    ...
    while ($rows = sql_fetch_array($result)) {
      ...
      array_push($user_data_array, $arrData);
  }
...
function get_geolocation_google($address){
    // $address = "건어물";
    $Url="https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($address)."&key=AIzaSyAjJ7dzV36r1sAhcBCuMX-MzP6IRalDALg";
    $ch = curl_init();
    ...
    $lati = $json_data['results']['0']['geometry']['location']['lat'];
    $lngi = $json_data['results']['0']['geometry']['location']['lng'];
    // $retrunData = $lati."^".$lngi;
    return $lati."^".$lngi;
}

function get_geolocation_kakao($address){
    // $address = "건어물";
    $key = "c6f22e6de670f5fad47ab435dde57f8e";
    $Url="https://dapi.kakao.com/v2/local/search/address.json?query=".urlencode($address);
    $headers = array('Authorization: KakaoAK c6f22e6de670f5fad47ab435dde57f8e');
    // $headers = "Authorization: KakaoAK $key"; 
    // $headers = array();
    // $headers = "Authorization: KakaoAK " . $key ."";
    // $headers = array('Authorization: KakaoAK . $key');
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $Url);
    ...
    curl_close($ch);
    $json_data = json_decode($output,true);
    $lati = $json_data['documents']['0']['address']['y'];
    $lngi = $json_data['documents']['0']['address']['x'];
    // echo "
    // lati > $lati <br>
    // lngi > $lngi <br>
    // ";
