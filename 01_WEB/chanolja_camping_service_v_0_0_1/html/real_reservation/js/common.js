...
function byName(nm, from) { // document.�쇰꽕�� �쇰줈 泥섎━�� 援ъ뒪�щ┰�� 泥섎━��
	...
	if(juso_api_use == 'Y' || juso_api_use == 'D') {
		...
		if (juso_api_use == 'D') {
			$.getScript('https://t1.daumcdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js', function() {
				...
				$.get('/main/exec.php?exec_file=common/zipSearch.php', function(r) { // �ㅽ궓
...
function checkLoginFrm(f){
	...
	if(f.setOpenSSL) { // �꾩뿭蹂��� setOpenSSL �� common.js �� �좎뼵�섏뼱�덉뒿�덈떎.
...
function previewResize(frameno, htype) {
	...
	if(typeof is_mobile != 'undefined' && is_mobile == 'Y') { // 紐⑤컮�쇱뿉�쒕뒗 媛�濡� 100%
...
function printReceipt(cate, cart_selected, ono) { // �λ컮援щ땲/二쇰Ц��/留덉씠�섏씠吏� �곸닔利� 異쒕젰
	var param = '';
	if(cate == 2 || cate == 3) {
	...
	nhn.husky.EZCreator.createInIFrame({
		...
		htParams : {bUseToolbar : true,
			fOnBeforeUnload : function() {
				//if(eType == 1) return '�섏씠吏�瑜� �대룞�섎㈃ ���ν븯吏� �딆� �댁뿭�� �좎떎�⑸땲��.';
			}
		},
		fOnAppLoad : function(){
...
function setDefaultFont() {
	var sDefaultFont = '沅곸꽌';
	var nFontSize = 24;
	oEditors.getById[contentId].setDefaultFont(sDefaultFont, nFontSize);
}

function prdMore(obj_id, file_name, module, option) { // �숇え諛붿씪 �붾낫湲� �ㅽ겕由쏀듃
	/*
	option.full_reload : true=�꾩껜�섏씠吏� �대룞, false=more �뺥깭
	*/
	...
	var _scroll_top = $(window).scrollTop(); // 援ш� �щ＼ 蹂�寃쎌궗�� ����
	$.get(root_url+'/main/exec.php?exec_file=skin_module/skin_ajax.php', param, function(result) {
		...
		if(option.nopaging != true) {
			if($.trim(result)) {
				...
				if(option.full_reload != true) { // more 踰꾪듉 �댁슜 �� �ㅼ쓬�섏씠吏�濡� 而ㅼ꽌 �대룞
					page++;
...
function cartPartCartAjax(pno, product_name, sell_prc) {
	$.post(root_url+'/main/exec.php', {"exec_file":"cart/cart.exe.php", "exec":"add", "pno":pno, "buy_ea":1, "from_ajax":true}, function(result) {
		if(/^[0-9]+$/.test(result) == true) {
			...
			if($('.quick_cart_parent').length > 0) { // �듭뭅�� �ъ슜��
	...
	if (typeof use_google_ads == 'boolean' && typeof google_ads_cart_id == 'string') {
		if (use_google_ads == true) {
			gtag('event', 'conversion', {
				...
				'currency': (currency == '��') ? 'KRW' : currency,
...
function commonAjaxUpload(input, url, param, complete) {
	if(!input.files) {
		window.alert('�ㅼ쨷 �낅줈�쒕� 吏��먰븯吏� �딅뒗 釉뚮씪�곗��낅땲��.\nIE10 �댁긽�� Internet Explorer �먮뒗 �щ＼ 釉뚮씪�곗�瑜� �댁슜�� 二쇱꽭��.');
		return false;
	}
