	...
	<table class="tbl_row multi_shop">
		...
		<tr>
          ...
          <table class="table table-bordered">
            <tr align="center" >
              <!-- <td>
                  <a href=<?php echo 'reservation_direct_reserve.php?year='.$preyear.'&month='.$month . '&day=1&top_menu_id=4001&left_menu_id=007'; ?>>◀◀</a>
              </td> -->
              <td>
                  <a href=<?php echo 'reservation_direct_reserve.php?year='.$prev_year.'&month='.$prev_month . '&day=1&top_menu_id=4001&left_menu_id=007'; ?>>◀</a>
              </td>
              <td height="50" bgcolor="#FFFFFF" colspan="5">
                  <a href=<?php echo 'reservation_direct_reserve.php?year=' . $thisyear . '&month=' . $thismonth . '&day=1&top_menu_id=4001&left_menu_id=007'; ?>>
                  <?php echo "&nbsp;&nbsp;" . $year . '년 ' . $month . '월 ' . "&nbsp;&nbsp;"; ?></a>
              </td>
              <td>
                  <a href=<?php echo 'reservation_direct_reserve.php?year='.$next_year.'&month='.$next_month.'&day=1&top_menu_id=4001&left_menu_id=007'; ?>>▶</a>
              </td>
              <!-- <td>
                  <a href=<?php echo 'reservation_direct_reserve.php?year='.$nextyear.'&month='.$month.'&day=1&top_menu_id=4001&left_menu_id=007'; ?>>▶▶</a>
              </td> -->
            </tr>
            <tr class="info">
                        ...
                        //   foreach ($room_info_array as $key=>$value) {
                          ...
                          <tr height="5px">
                        	<td colspan='2'>
                                ...
                                <span class="box_btn blue" data-toggle="modal" data-backdrop="static" data-whatever="<?=$reserve_date?>" staic data-target="#reserveModal">
                                <input type="button" id="client_reserve_input" value="<?=$rows['reserveNum']?>건"></span>
                                <?}else{?>
                                <span class="box_btn info" data-toggle="modal" data-backdrop="static" data-whatever="<?=$reserve_date?>" staic data-target="#reserveModal">
                                <input type="button" id="client_reserve_input" value="<?=$rows['reserveNum']?>건"></span>
                                <?}?>
                            </td>
                        	</tr>
    ...
    // $('#payment_method_array').val(payment_method_array);
					// var result = confirm("등록/변경 하시겠습니까?");
						 // if(result) { //확인 클릭 시
							...
							//  sh.action = "reservation_direct_reserve_action.php";
							//  sh.submit();
							// }
	//  });
</script>
<script type="text/javascript">
  ...
  //    }
	...
	// 	}
	// }
    function direct_reserve(){
        var todate = "<?=$todate?>";
        var direct_reserve_date = $("#reserve_date").val();

        console.log('date : ',todate, direct_reserve_date);

        var temp = new Date(direct_reserve_date);
        var DateStr = `${temp.getFullYear()}-${('00' + (temp.getMonth()).toString()).slice(-2)}-${('00' + (temp.getDate()+1).toString()).slice(-2)}`;
        console.log(temp, DateStr);
        
        if(direct_reserve_date < todate){
            alert('예약이 불가능한 날짜입니다.');
            return false;
        }
        else{
            location.href='reservation_admin_form.php?&direct_reserve_date='+DateStr+"&top_menu_id=4001&left_menu_id=014";
        }
        alert('direct_reserve_date ' + direct_reserve_date);
...
<div class="modal fade" id="reserveModal" tabindex="-1" role="dialog" aria-labelledby="reserveModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-fullsize" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">닫기</button>
        <button type="button" class="btn btn-primary" onclick="direct_reserve();">예약</button>
      </div>
    </div>
  </div>
</div>
