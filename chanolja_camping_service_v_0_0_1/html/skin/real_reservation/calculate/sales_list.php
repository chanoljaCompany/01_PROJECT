...
$sql = "select user_id from guestroom_reservation
        where 1=1
        AND guestroom_reception_date = '2022-03-21'
        group by user_id";
		...
		<section id="wrapper">
				<article id="contentArea">
					...
					<table class="tbl_row multi_shop">
						<caption class="hidden">매출현황</caption>
						<colgroup>
							<col style="width:15%">
							<col style="width:85%">
						</colgroup>
                        <tr>
							<th scope="row">상태</th>
							<td>
								<label class="p_cursor"><input type="radio"  class="searchVal_44" checked> 예약완료</label>
							</td>
						</tr>
						<tr>
							<th scope="row">매출번호/상품명</th>
							<td>
								<select id="searchKey_1" name="searchKey_1">
												<option value="guestroom_reserve_code" selected>매출번호</option>
												<option value="guestroom_name">상품명</option>
								</select>
								<input type="text" name="searchVal_1" id ="searchVal_1" class="input" size="30" placeholder="매출번호 또는 상품명">
							</td>
						</tr>
						<!-- <tr>
							<th scope="row">기간</th>
							<td>
								<select id="searchKey_2" name="searchKey_2">
												<option value="guestroom_reserve_date_start" selected>매출일자</option>
						...
						<!-- <tr>
							<th scope="row">매출상태</th>
							<td>
								<label class="p_cursor"><input type="checkbox" name="searchVal_6[]" class="searchVal_6" value="2" checked> 매출신청</label>
								<label class="p_cursor"><input type="checkbox" name="searchVal_6[]"  class="searchVal_6" value="3" checked> 매출완료</label>
...
function showTable(curpage) {
  ...
  $("#curpage").val(curpage);
	...
	$.ajax({
		type: 'POST',
		url:'./sales_search_list_ajax.php',
	...
	function showPaing(curpage) {
	        ...
	        $.ajax({
	            type: "POST",
	            url: "./sales_search_list_paging_ajax.php",
	            // data: "searchVal_1="+searchVal_1+"&searchVal_2="+searchVal_2+"&searchVal_3="+searchVal_3+"&searchVal_4="+searchVal_4_array+"&searchVal_5="+searchVal_5+"&searchVal_6="+searchVal_6_array+"&curpage=" + curpage,
							enctype: 'multipart/form-data',
							data:formData,
							cache:false,
	...
	function go_page(division,guestroom_reserve_code){//페이지구분,상품고유번호or상품매출번호,날짜,방이름,매출상태
